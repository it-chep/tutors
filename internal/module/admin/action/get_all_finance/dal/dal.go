...
func (r *Repository) GetFinanceInfo(ctx context.Context, from, to time.Time, adminID int64) (decimal.Decimal, error) {
	...
	perHours := r.perHours(ctx, adminID)
	...
	for _, tutorInfo := range perHours {
		tutorsLessons, ok := tutorsMap[tutorInfo.TutorID]
		if !ok {
			continue
		}
...
func (r *Repository) perHours(ctx context.Context, adminID int64) (moneys []dao.StudentTutorMoney) {
	perHourSQL := `
